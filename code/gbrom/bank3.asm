...
_Call_7FF0:: ; 6662
	; psobot: this ends up getting called on every timer interrupt
	push af
	push bc
	push de
	push hl

	; psobot: tell stm32 about current level
	ldh a, [hLevelIndex]
  ld hl, AUDIO_REGISTER_LEVEL
  ldd [hl], a
  ; and same with game state
  ldh a, [hGameState]
  ld hl, AUDIO_REGISTER_GAME_STATE
  ldd [hl], a
  ; /psobot
...
.pauseMusic
	...
	; psobot: pause STM32
  ld a, AUDIO_COMMAND_PAUSE
  ld hl, AUDIO_REGISTER_COMMANDS
  ldd [hl], a
  ; /psobot
.playFirstNote
...
.unpauseMusic
	xor a
	ldh [hPauseTuneTimer], a
	; psobot: unpause STM32
  ld a, AUDIO_COMMAND_UNPAUSE
  ld hl, AUDIO_REGISTER_COMMANDS
  ldd [hl], a
  ; /psobot
	jr .jmp_6688
...
StartMusic:: ; 6AB5
	...
	; ld a, b
	; and a, $1F			; Huh?
	; psobot: turn the external sound input on
	; ld hl, rNR50
	; ld a, $BB
 ;  ldd [hl], a
  ; psobot: turn internal sound off
 ;  ld hl, rNR51
	; ld a, $00
 ;  ldd [hl], a
 ;  ld hl, rNR52
 ...
 ;  ldd [hl], a
  ; /psobot
  ; psobot: turn on audio player
	ld a, AUDIO_COMMAND_PLAY
  ld hl, AUDIO_REGISTER_COMMANDS
  ldd [hl], a
 	; /psobot
	call LookupSoundPointer.lookup
